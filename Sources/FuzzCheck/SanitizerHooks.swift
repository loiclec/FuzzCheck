...
func trace_pc_guard(g: UnsafeMutablePointer<UInt32>?) {
    guard TraceProgramCounter.shared.isRecording, let g = g else { return }
    let pc = PC(bitPattern: __return_address())
    let idx = Int(g.pointee)
    TraceProgramCounter.shared.edges[idx] = pc
    let (result, overflow) = TraceProgramCounter.shared.eightBitCounters[idx].addingReportingOverflow(1)
    if !overflow {
        TraceProgramCounter.shared.eightBitCounters[idx] = result
    }
}
...
func trace_pc_guard_init(start: UnsafeMutablePointer<UInt32>, stop: UnsafeMutablePointer<UInt32>) {
    TraceProgramCounter.shared.handlePCGuardInit(start: start, stop: stop)
}

@_cdecl("__sanitizer_cov_trace_pc_indir")
func trace_pc_indir(callee: PC) {
    guard TraceProgramCounter.shared.isRecording else { return }

    let caller = PC(bitPattern: __return_address())
    TraceProgramCounter.shared.handlePCIndir(caller: NormalizedPC(caller), callee: NormalizedPC(callee))
}
...
func trace_cmp8(arg1: UInt64, arg2: UInt64) {
    ...
    TraceProgramCounter.shared.handleTraceCmp(pc: pc, arg1: arg1, arg2: arg2)
}
...
func trace_const_cmp8(arg1: UInt64, arg2: UInt64) {
    ...
    TraceProgramCounter.shared.handleTraceCmp(pc: x, arg1: arg1, arg2: arg2)
}
...
func trace_cmp4(arg1: UInt32, arg2: UInt32) {
    ...
    let pc = PC(bitPattern: __return_address())
    TraceProgramCounter.shared.handleTraceCmp(pc: NormalizedPC(pc), arg1: arg1, arg2: arg2)
}
...
func trace_switch(val: UInt64, cases: UnsafePointer<UInt64>) {
    ...
    TraceProgramCounter.shared.handleTraceCmp(pc: NormalizedPC(pc + UInt(i)), arg1: token, arg2: 0)
}
...
func trace_div4(val: UInt32) {
    ...
    let pc = PC(bitPattern: __return_address())
    TraceProgramCounter.shared.handleTraceCmp(pc: NormalizedPC(pc), arg1: val, arg2: 0)
}
